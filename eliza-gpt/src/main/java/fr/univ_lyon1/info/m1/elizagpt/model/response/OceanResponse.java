 ...
 */
public class OceanResponse implements Response {
    ...
    final List<Verb> listVerb) {
        Pattern pattern = Pattern.compile("Quel est le plus grand océan du monde \\?", 
        Pattern.CASE_INSENSITIVE);
        Matcher matcher = pattern.matcher(normalizedText);
        if (matcher.matches()) {
            String result = ("Le plus grand océan du monde est l'océan Pacifique.");
